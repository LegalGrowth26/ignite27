...
export async function GET(): Promise<Response> {
  ...
  const { data, error } = await client
    ...
    .select(
      `booking_reference, company_name, company_contact_name,
       ...
       exhibitor_requirements (needs_power, needs_table_chairs, signage_name, website_url, approved_at),
  ...
  const csv = toCsv<Row>(rows, [
    ...
    { header: "Approved for public site", value: (r) => (r.exhibitor_requirements?.approved_at ? "yes" : "no") },
