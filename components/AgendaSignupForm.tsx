...
import { subscribeAgendaUpdatesAction } from "@/app/agenda/actions";
...
export function AgendaSignupForm() {
  const [email, setEmail] = useState("");
  const [wantsAgendaAlert, setWantsAgendaAlert] = useState(false);
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!wantsAgendaAlert) {
      setError("Tick the box to confirm you want the agenda alert.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="flex flex-col gap-5">
      <div>
        <label htmlFor="agenda-email" className={LABEL}>
      ...
      <label className="flex items-start gap-3">
        <input
          type="checkbox"
          name="wantsAgendaAlert"
          checked={wantsAgendaAlert}
          onChange={(e) => setWantsAgendaAlert(e.target.checked)}
          required
          className="mt-1 h-4 w-4"
        />
        <span className="text-small text-ignite-ink">
          Notify me when the Ignite 27 agenda is announced. <span className={REQ}>*</span>
        </span>
      </label>
